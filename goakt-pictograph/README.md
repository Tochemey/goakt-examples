...
This is the multi-user companion piece to [`goakt-tetris`](../goakt-tetris): where Tetris is a tick-driven single-player actor, Pictograph is an event-driven, multi-writer, FSM-shaped actor whose broadcast path runs through GoAkt's pub/sub topic actor.
...
| Static discovery (configurable seed peer list, same shape as `goakt-tetris`)     | `main.go::peerList`                                                              |
...
Discovery is **static**, identical to `goakt-tetris`: pass `--peers host:9001,host:9101` to each node. Swap in `discovery/kubernetes` or `discovery/dnssd` for production — only `main.go::buildActorSystem` would change.
...
| `Dockerfile` + `docker-compose.yaml` | Two-node cluster image + service definition (same layout as `goakt-tetris`)                                             |
